...
def test_dcall_empty_target(msig, usrs):
    action = Action(CallType.DELEGATECALL, C.ADDRESS_EMPTY, 3000, 0, '0xdeadbeefab')
    signAndExecute(msig, usrs, action)
    assert msig.nonce() == 1

def test_dcal_value_and_data(msig, mock, usrs):
    value = 100
    action = Action(CallType.DELEGATECALL, mock.address, 4000, value, '0xdeadbeef')
...
def test_fail_unmatched_value(msig, usrs):
    value = 100
    action = Action(CallType.DELEGATECALL, C.ADDRESS_EMPTY, 3000, value, C.EMPTY_BYTES)
    with brownie.reverts():
        signAndExecute(msig, usrs, action)
    with brownie.reverts():
